...
- **Fase 3 mulai (2026-09-26):** 3a Dashboard Agent `/agent` selesai (`components/agent/DashboardView.tsx`, data `lib/agent/dashboard-data.ts`, `components/ui/StatusBadge.tsx`, `app/agent/loading.tsx`, galeri `/komponen/agent`); belum diuji dengan login Agent. 3b Profil Saya `/agent/profil` selesai (`components/agent/{ProfileForm,KtpCard}.tsx`, `lib/agent/profile-data.ts`, `lib/validation/profile-form.ts`, `components/ui/{Switch,Field.Select}`, galeri `?layar=profil`); belum diuji dengan login Agent. Urutan berikut: 3c Listing Saya/Wizard/Detail, 3d Pembelajaran/Belajar-Course, 3e Event, 3f Organisasi; satu PR per modul.
