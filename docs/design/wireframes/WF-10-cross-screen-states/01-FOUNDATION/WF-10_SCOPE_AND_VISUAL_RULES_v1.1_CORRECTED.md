# WIRE-10 Scope & Visual Rules v1.1 CORRECTED
...
## 16. Create Listing field boundary — explicit WIRE-10 exclusion
The Core-required Create Listing fields are owned by WIRE-02/M03. WIRE-10 must not recreate
their domain input controls or redefine their semantic, dependency, requiredness, conditionality,
ownership, editability, or lifecycle behavior.

The 13 required user-facing Create Listing fields are:
1. listing_context
2. category
3. transaction_type
4. title
...
Their authoritative detailed UX treatment remains in WIRE-02. WIRE-10 only supplies reusable
cross-screen state presentation patterns (for example inline validation, loading, error, pending,
success, denied, and recovery) that WIRE-02 may apply to those fields. This is a shared visual/state
pattern, not a transfer of field ownership.

Accordingly, a WIRE-10 screen is not considered incomplete because these 13 fields are absent from
its own visual inventory. Reproducing them inside WIRE-10 would be scope duplication and authority
inversion.
