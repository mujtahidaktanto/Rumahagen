...
3. **[DITUTUP 0140/0141, 0141 diterapkan 2026-09-24] Jenis kapasitas `listing_slot` kini ada** (kuota penerbitan listing; slot beli tidak reset dan tidak kedaluwarsa sebelum dipakai; pembelian dengan organisasi memberi slot ke organisasi). Teks lama: jenis yang dikonsumsi hanya `listing_refresh` dan `learning_point`. Form Add-on kini menawarkan Slot Listing. Tidak ada penegakan "slot listing" di mana pun (dicek seluruh kode dan migration), padahal wireframe Katalog Agent menampilkan add-on "+25 slot listing aktif". Add-on jenis itu belum bisa dijual sampai konsumennya dibangun; form hanya menawarkan dua jenis di atas.
...
5. **[DITUTUP 0138] `commercial_orders.organization_id` kini diverifikasi keanggotaannya.**
...
## 6. Aturan kelayakan promosi (2026-09-24, migration 0134 diterapkan)
Aturan kelayakan kini dirancang dievaluasi server (kunci: roles, first_purchase_only, max_redemptions, max_redemptions_per_user, min_list_price). Layar Admin: **Form Promosi** memuat kartu Aturan kelayakan (chip peran, sakelar pembelian pertama, batas total/per pengguna, harga minimum, ringkasan, bilah kuota terpakai pada mode edit, dan contoh harga yang menghormati harga minimum); **daftar Promosi** memuat kolom Kelayakan dan Pemakaian (mis. 37 / 100, lencana Kuota habis); **Form Add-on** menampilkan ringkasan kelayakan pada kartu pilihan promosi. Pembeli mendapat kelayakan lewat `promotion_offer` pada katalog/offers/products (berlaku, alasan, harga akhir). `rule_configuration` (migration 0145) kini dievaluasi; Form Promosi memuat kartu **Aturan tambahan**: batas potongan maksimum, cakupan pembelian (pribadi/organisasi), produk tertentu (kode dipisah koma), hari berlaku, jam mulai/selesai WIB, dan pengguna baru dalam N hari, dengan ringkasan dan contoh harga yang menghormati batas potongan.
