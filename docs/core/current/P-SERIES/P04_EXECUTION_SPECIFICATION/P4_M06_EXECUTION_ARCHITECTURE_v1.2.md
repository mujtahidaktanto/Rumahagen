# P4 Module Execution Specification Architecture — M06 v1.3
**Authority:** Developer/Project/Marketing Kit/Claim  
**Status:** FULL-VERSION / CURRENT SUCCESSOR EXECUTION ARCHITECTURE

## 1. Authority and Execution Boundary
- `M06` remains the semantic authority for its domain: **Developer/Project/Marketing Kit/Claim**.
...
- **AUGMENT** | Developer company_logo and free-text “Tentang Developer” description are semantic source requirements. | `M06-P4-WP01` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
- **PRESERVE** | Project keeps meta_title/meta_description; do not introduce Project description as substitute. | `M06-P4-WP02` | consequence: Carry unchanged into execution contract and regression acceptance.
- **RECONCILE** | Project media semantic boundary is photo/video; brochure/price_list physical allowance must not override semantics. | `M06-P4-WP03` | consequence: Resolve naming/authority/contract interpretation before task execution; preserve valid predecessor detail.
- **ADD-NEW** | Marketing Kit is a separate semantic resource; permissions: Developer own CRUD; Admin/Superadmin All; Manager/Agent View+Download; Buyer/Partner none. | `M06-P4-WP03` | consequence: Create an explicitly scoped execution work package/task within module authority.
- **RECONCILE** | Approved Claim → Agent-owned Listing initialization must preserve M03 Listing authority. | `M06-P4-WP03` | consequence: Resolve naming/authority/contract interpretation before task execution; preserve valid predecessor detail.
...
- **M06-P4-WP01 — Developer/Project Core**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M06-P4-WP02 — Project Media & SEO**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M06-P4-WP03 — Marketing Kit**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M06-P4-WP04 — Claim → Listing Handoff**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
