# P4 Module Execution Specification Architecture — M02 v1.3
**Authority:** Profile/Review  
**Status:** FULL-VERSION / CURRENT SUCCESSOR EXECUTION ARCHITECTURE

## 1. Authority and Execution Boundary
- `M02` remains the semantic authority for its domain: **Profile/Review**.
...
- **RECONCILE** | Profile PUBLIC/PRIVATE must be a visibility state, not an RBAC scope. | `M02-P4-WP03` | consequence: Resolve naming/authority/contract interpretation before task execution; preserve valid predecessor detail.
- **AUGMENT** | Public/WhatsApp CTA requires explicit Agent opt-in. | `M02-P4-WP03` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
- **PRESERVE** | Only Superadmin may administratively override another Agent profile visibility/Public CTA. | `M02-P4-WP03` | consequence: Carry unchanged into execution contract and regression acceptance.
- **RECONCILE** | Reviews auto-approve for publication; Admin moderation is post-publication. | `M02-P4-WP03` | consequence: Resolve naming/authority/contract interpretation before task execution; preserve valid predecessor detail.
...
- **M02-P4-WP01 — Profile Core**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M02-P4-WP02 — Review Lifecycle**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M02-P4-WP03 — Visibility & CTA**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M02-P4-WP04 — Profile Presentation & Validation**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
