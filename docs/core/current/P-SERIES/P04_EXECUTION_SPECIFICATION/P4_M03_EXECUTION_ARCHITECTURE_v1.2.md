# P4 Module Execution Specification Architecture — M03 v1.3
**Authority:** Listing/Lifecycle/Refresh  
**Status:** FULL-VERSION / CURRENT SUCCESSOR EXECUTION ARCHITECTURE

## 1. Authority and Execution Boundary
- `M03` remains the semantic authority for its domain: **Listing/Lifecycle/Refresh**.
...
- **RECONCILE** | Normal Publish must not use a Pending Review gate or Admin approval gate. | `M03-P4-WP02` | consequence: Resolve naming/authority/contract interpretation before task execution; preserve valid predecessor detail.
- **PRESERVE** | Suspended is enforcement/ethics state, separate from publication approval. | `M03-P4-WP01` | consequence: Carry unchanged into execution contract and regression acceptance.
- **AUGMENT** | Sold/Rented excluded from public Search while retained in historical Agent-profile counts. | `M03-P4-WP01` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
- **PRESERVE** | Ordinary Listing Update is owner-only; Organization membership does not create edit authority. | `M03-P4-WP04` | consequence: Carry unchanged into execution contract and regression acceptance.
- **AUGMENT** | Address, Property Type, Land Size, Building Size lock after first successful Publish. | `M03-P4-WP02` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
- **AUGMENT** | Refresh default is 5 successful Refreshes per Agent per operational day; value must remain configurable. | `M03-P4-WP03` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
- **AUGMENT** | No carry-forward; Asia/Jakarta operational-day reset; one successful Refresh per Listing/day. | `M03-P4-WP03` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
- **AUGMENT** | Refresh remains within existing District; ordering latest successful first; timestamp tie first-recorded; server authoritative. | `M03-P4-WP03` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
- **AUGMENT** | Search cards use optimized derivatives; preserve aspect ratio; no original-resolution ordinary-card delivery. | `M03-P4-WP01` | consequence: Extend the execution contract/state/task acceptance without deleting inherited capability.
...
- **M03-P4-WP01 — Listing Core/Lifecycle**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M03-P4-WP02 — Publish & Field Locks**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M03-P4-WP03 — Refresh Action & Allowance Contract**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
- **M03-P4-WP04 — Search/Media/Validation**: dependency gate → contract definition → atomic tasks → validation → evidence → checkpoint → handoff.
