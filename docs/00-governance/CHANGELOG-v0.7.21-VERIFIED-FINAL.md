...
**`0.7.21`** — *Initial Development*
Dirilis: 2026-08-11
Fase: Pra-Development — **Penetapan resmi nama brand/produk: RUMAHAGEN**, menggantikan nama kerja generik "Real Estate Agency Platform"/placeholder `{nama_platform}`, keputusan langsung Business Owner (`decision-log.md` OD-26). Rename murni redaksional/branding — diterapkan ke isi dan nama file di seluruh dokumen governance, produk, teknis, dan migration; **tidak ada perubahan skema, kontrak API, atau keputusan arsitektur**.

*(Riwayat: `0.7.20`, dirilis 2026-08-10 — audit ulang MP-02 standar verifikasi ketat penuh, tidak ada regresi, rasio regresi sistemik final 50%, PATCH. `0.7.19`, dirilis 2026-08-10 — konsolidasi riwayat versi 13 Module Planning + SYSTEM-ARCHITECTURE.md + AI Development Blueprint selesai penuh, 7 regresi ditemukan & diperbaiki, PATCH. `0.7.2`, dirilis 2026-08-07 — OD-24, gate M12 terbuka, PATCH.)*
...
## [0.7.21] - 2026-08-11 - Penetapan Nama Brand/Produk Resmi: RUMAHAGEN

### Added
Tidak ada perubahan pada kategori ini di rilis ini.

### Changed
- **Nama brand/produk final ditetapkan: RUMAHAGEN.** Menggantikan nama kerja
  generik "Platform Web Real Estate Agency" / "Real Estate Agency Platform" /
  "Real-Estate-Agency-Platform" yang dipakai sejak dokumen pertama proyek ini
  disusun, dan yang secara eksplisit tercatat sebagai placeholder belum final
  (`{nama_platform}`) di `AI-CONTEXT-PACK.md` dan seluruh snapshot
  `CURRENT-PROJECT-STATE.md`. Keputusan langsung Business Owner, dicatat
  sebagai **OD-26** di `decision-log.md` §11 (RESOLVED, mengikuti pola
  bridging OD-02/OD-06/OD-25 — administratif/branding, tidak memerlukan ADR
  arsitektur baru).
- **Cakupan penggantian:** isi maupun nama file di ±84 dokumen governance,
  produk, teknis, dan migration — termasuk 12 nama file berpola
  `{Jenis-Dokumen}-Real-Estate-Agency-Platform-v{X.Y}` menjadi
  `{Jenis-Dokumen}-RUMAHAGEN-v{X.Y}` (struktur/konvensi penamaan file **tidak
  berubah**, murni token nama produk yang diganti).
- **Sengaja tidak diganti:** deskripsi kategori/vertikal bisnis "PropTech /
  Real Estate Agency SaaS" (`PROJECT-CONSTITUTION.md`, `AI-CONTEXT-PACK.md`)
  — mendeskripsikan jenis industri produk, bukan nama brand.
- **Dokumen governance disinkronkan pada siklus yang sama:**
  `document-governance-baseline-register.md` naik v1.10→**v1.11**
  (Governance Notes poin 39); `project-manifest.md` naik v1.27→**v1.28**
  (Governance Notes poin 16); `decision-log.md` §11 (**OD-26**, baru).
...
### Database Changes
Tidak ada perubahan skema — rename murni redaksional/branding, tidak ada
migration baru maupun perubahan struktur tabel/kolom/RLS.

### API Changes
Tidak ada perubahan kontrak endpoint pada rilis ini. Nama file dokumen acuan
(`API-Specification-...md`) berubah mengikuti rename brand — isi kontrak
tidak berubah.

### UI Changes
Tidak ada perubahan pada kategori ini di rilis ini — perubahan nama brand
pada salinan/UI teks aktual (label produk yang tampil ke pengguna) adalah
pekerjaan implementasi terpisah, belum dieksekusi karena kode aplikasi belum
diinisialisasi (0% kode, konsisten `CURRENT-PROJECT-STATE.md`).

### Action Required
- **Owner/tim disarankan** meninjau ulang nama domain, akun pihak ketiga
  (Google Search Console, GTM, GA4, dsb. — lihat Open Decision #7
  `decision-log.md`), dan aset branding lain (logo, favicon, metadata SEO)
  agar konsisten dengan nama final RUMAHAGEN sebelum Sprint S0 dimulai.
- Rujukan nama produk di Bagian 4/5/8/19 `project-manifest.md` (nama file
  dokumen lama) direkomendasikan disinkronkan penuh pada siklus governance
  berikutnya — dicatat sebagai gap terbuka di Governance Notes poin 16
  dokumen tsb, bukan diperbaiki diam-diam.
