...
| 1.11 | 17 Agu 2026 | P08 canonical synchronization governance: explicit separation of decision, semantic, intended physical, executed physical, and runtime-verified state; canonical source/authority propagation; historical preservation; implementation-readiness boundary; repository promotion is a separate operational activity and is not a document-completion gate |
...
### Repository Promotion / Local Artifact Authority Rule

During the controlled canonical synchronization process, repository hosting is an **evidence/reference layer**, not a completion gate for document authoring.

The following rules are binding:

1. A controlled document is considered **complete for the current synchronization step** when its content has been reconciled against the approved sources, internally validated, versioned according to its document lifecycle, and saved as the final local artifact.
2. GitHub repository write access or post-write GitHub verification is **not required** to declare the local synchronization artifact complete.
3. When GitHub is operated as read-only reference during a synchronization pass, the finalized local artifacts are the authoritative deliverables of that pass and may subsequently be promoted by the project owner through the approved local Git/Git Bash workflow.
4. Failure to create, update, or verify a GitHub file must **not** cause a completed local artifact to remain classified as `OPEN`, `FAILED`, or `INCOMPLETE`, provided the document itself has passed its reconciliation and validation gate.
5. GitHub promotion is a separate operational activity and must not be conflated with semantic reconciliation, document validation, or architecture approval.
6. Historical repository artifacts must remain preserved unless a separately authorized repository maintenance action requires otherwise.
7. A repository synchronization failure may be recorded as an **operational follow-up**, but it does not reopen an approved semantic decision or invalidate the completed local document.

This rule applies to the current Wave 1 controlled synchronization and to later synchronization passes unless a higher-level governance decision explicitly changes it.
...
**Synchronization step:** STEP 04  
**Effective date:** 16 August 2026  
**Scope:** D0 Authority & Vocabulary + D1 Commercial/Payment semantic foundation  
**Status:** Synchronized semantic baseline; physical implementation remains downstream.

## A. Governing hierarchy clarification
For the explicit architecture-evolution boundaries governed by approved MADCR/ADR decisions, the governing decision remains authoritative. The existing Constitution remains the binding rule-set for downstream work. AEP documents are controlled translation/impact artifacts and are **not** independent sources of truth.

The effective interpretation is:

1. Approved ADR/MADCR decision within its explicit scope.
2. `PROJECT-CONSTITUTION.md` for binding downstream project rules.
3. `technology-decisions.md` for technology selection/rationale.
4. `SYSTEM-ARCHITECTURE.md` for integrated technical architecture.
5. Downstream ERD/API/RBAC/PRD/User Flow/Schema artifacts.
6. AEP impact packages as traceability/evidence, not as independent authority.

A newer approved MADCR/AEP evolution is not used to silently rewrite historical ADR text; it is applied as a controlled overlay to the affected downstream baseline.

## B. Canonical D0 vocabulary
The following cross-AEP terms are now canonical for downstream synchronization:

- **Commercial Entitlement** is distinct from **Subscription** and **RBAC**.
- **Payment Core** is provider-independent; **Provider Adapter** is the integration boundary.
- **Provider callback/webhook receipt** is not equivalent to **Payment Confirmed**.
- **Verification + idempotency** precede commercial fulfillment.
- **Commercial Entitlement → Quota Capacity → Operational Pool → Allocation/Usage** is the quota authority chain.
- **Learning Session** is a semantic learning capability; provider session IDs and meeting links are infrastructure references.
- **Certificate/Credential** is distinct from **Award Instance**.
- **Event Registration** is distinct from **Session Enrollment**.
- Unresolved MADCR/Open Decision items remain explicitly unresolved.

## C. D1 Commercial/Payment foundation
The synchronized architecture must preserve these invariants:

1. Payment is a subdomain within Commercial (M14) under MADCR-011.
2. Commercial Entitlement is authoritative for agency/organization quota capacity under MADCR-010.
3. Payment Core remains provider-independent under MADCR-002.
4. Verified and idempotent payment fulfillment is required under MADCR-003.
5. Historical commercial purchase terms are commercial-complete and immutable after confirmation under the approved MON-006 decision.
6. Payment does not directly mutate RBAC.
7. Reconciliation detects/investigates state divergence; a reconciliation finding does not automatically authorize entitlement/quota/subscription mutation.
8. Configurable commercial parameters are configuration/data, not hard-coded business constants, and changes do not rewrite historical purchases/grants.

## D. Explicit non-decisions
This synchronization does **not** lock:
- exact table names/keys/cardinalities/indexes;
- exact payment state enum;
- exact webhook/API endpoint or transport mechanics;
- exact idempotency storage/key/locking/retry implementation;
- exact RBAC permission IDs;
- provider-specific credentials or provider selection;
- physical migration SQL.

Those remain downstream of the controlled technical synchronization gates.
