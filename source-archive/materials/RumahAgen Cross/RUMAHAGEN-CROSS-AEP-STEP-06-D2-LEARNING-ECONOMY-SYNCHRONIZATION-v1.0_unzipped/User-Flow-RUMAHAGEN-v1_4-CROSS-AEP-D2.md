...
### D2-FLOW-01 — Internal Learning Economy journey
`Learn / Complete Activity → Validated Completion → Earn LP → LP Ledger / Provenance → Progression / Unlock → Assessment where required → Skill/Credential outcome where governed`.

### D2-FLOW-02 — Purchased LP journey
`Select LP Package → Commercial Order/Payment → Trusted Confirmed Payment → One Purchased LP Grant → LP Ledger / Provenance → Progression / Unlock`.

Repeated callback/retry must not create duplicate economic grants.

### D2-FLOW-03 — Pay-to-Accelerate boundary
Purchased LP may accelerate progression, but mandatory assessment/competency cannot be bypassed.

### D2-FLOW-04 — Partnership Learning
Partner learning may use a conventional partner model. Partner results/credentials retain partner provenance and are not silently converted into Internal Learning Economy outcomes.

### D2-FLOW-05 — Completion boundary
Activity completion is not itself a verified Skill/Credential. The applicable rule/awarding path and assessment/competency gate determine outcome.

### D2-FLOW-06 — MADCR-049 hold
Where Learning Activity evidence is unresolved, user-visible and API semantics must not imply that the final evidence governance is already closed.
