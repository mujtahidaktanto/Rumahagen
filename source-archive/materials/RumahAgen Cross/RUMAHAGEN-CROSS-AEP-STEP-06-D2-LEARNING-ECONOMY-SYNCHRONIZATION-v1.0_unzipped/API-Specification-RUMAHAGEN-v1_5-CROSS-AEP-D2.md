...
### D2-API-01 — Learning Economy contract
Canonical capability flow:

`Learning Activity / Learning Action → Validated Completion → Earned LP Transaction → Progression / Unlock`

and, for purchased points:

`Confirmed Commercial Payment → Purchased LP Grant → LP Transaction / Provenance → Progression / Unlock`

### D2-API-02 — Mandatory contract invariants
- Payment confirmation is consumed from Commercial; Learning does not implement Payment Gateway logic.
- Purchased LP grant must be idempotent against the confirmed commercial payment transaction.
- Earned and Purchased LP provenance must remain distinguishable.
- Completion alone must not automatically issue verified Skill/Credential.
- Mandatory assessment/competency requirements cannot be bypassed by purchased LP.
- Partner credentials/results retain partner provenance.
- Configuration changes do not mutate historical economic records.
- Learning Points are not RBAC permissions and not Commercial Entitlement.

### D2-API-03 — MADCR-049 hold
Activity/evidence endpoints and exact evidence semantics remain conditional where they depend on unresolved MADCR-049. No endpoint introduced here is closure evidence for MADCR-049.

### D2-API-04 — Physical/API hold
Exact endpoint paths, payloads, status enums, idempotency key format/storage and evidence transport remain downstream.
