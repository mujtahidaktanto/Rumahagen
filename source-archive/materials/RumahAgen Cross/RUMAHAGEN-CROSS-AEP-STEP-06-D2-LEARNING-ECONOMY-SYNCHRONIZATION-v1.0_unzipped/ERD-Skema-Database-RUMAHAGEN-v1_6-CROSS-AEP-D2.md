...
### D2-ERD-01 — Learning Economy logical domain
The downstream logical model must represent Learning Points as a first-class transaction/provenance domain, not as a mutable balance-only concept.

Required logical capabilities:
- Learning Point transaction / ledger;
- Earned vs Purchased provenance;
- Learning Activity / completion evidence reference;
- Learning Path / progression / unlock;
- Assessment / competency gate;
- Skill;
- Credential;
- Partnership result/credential provenance;
- Learning Economy configuration;
- purchase/grant correlation to confirmed Commercial payment;
- idempotent purchased-LP grant identity;
- audit/provenance.

### D2-ERD-02 — Canonical boundaries
1. `Learning Point ≠ Commercial Entitlement`.
2. `Learning Completion ≠ Skill/Credential`.
3. Purchased LP can accelerate progression but cannot bypass mandatory assessment/competency.
4. Internal Learning uses Learning Economy; Partnership Learning does not have to use the internal LP economy.
5. Payment remains Commercial-owned; Learning consumes a trusted confirmed-payment outcome.
6. One confirmed payment may produce only one confirmed purchased-LP grant.
7. Configuration changes must not mutate historical LP transactions/grants/purchases/provenance.
8. Certificate/Credential remains distinct from Title/Award Instance.

### D2-ERD-03 — MADCR-049 conditional boundary
Learning Activity may be represented as a semantic/business-rule concept and referenced by downstream design, but its final evidence/governance model remains conditional on MADCR-049. No ERD element created by this step may be interpreted as closure evidence.

### D2-ERD-04 — Physical hold
Do not lock exact table names, keys, cardinalities, indexes, state enums, evidence storage, idempotency mechanism or migration SQL in this step.
