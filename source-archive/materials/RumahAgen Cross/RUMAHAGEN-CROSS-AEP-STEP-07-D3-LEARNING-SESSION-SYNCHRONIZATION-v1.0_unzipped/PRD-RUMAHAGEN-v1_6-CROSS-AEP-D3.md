...
### D3-PRD-01 — Canonical Learning Session capabilities
Product shall support:
- BROADCAST, INTERACTIVE, ON_DEMAND session taxonomy;
- DRAFT/SCHEDULED/LIVE/ENDED lifecycle with CANCELLED/FAILED exceptions;
- separate Session Enrollment PENDING/ACTIVE/COMPLETED;
- session discovery in Learning, with Event Calendar as integration/presentation context;
- provider-neutral session access through adapter boundary;
- normalized participation evidence;
- governed attendance and completion;
- optional recording/artifact;
- provider switching preserving semantic identity/history;
- paid access consuming Commercial outcomes;
- Learning Activity handoff without direct LP issuance;
- audit/provenance.

### D3-PRD-02 — Acceptance invariants
- M04 Course Enrollment is not reused.
- M05 EventRegistration is not reused.
- Provider Session ID is not product identity.
- Provider participation is evidence, not attendance/completion authority.
- Completion does not automatically create LP, Credential, Certificate or Title/Award.
- Automatic provider failover is not assumed.
- Exact attendance formula/capacity/visibility/recording policy remain downstream.

### D3-PRD-03 — MADCR-049
Learning Activity handoff is defined as a boundary, but final evidence semantics remain conditional on MADCR-049.
