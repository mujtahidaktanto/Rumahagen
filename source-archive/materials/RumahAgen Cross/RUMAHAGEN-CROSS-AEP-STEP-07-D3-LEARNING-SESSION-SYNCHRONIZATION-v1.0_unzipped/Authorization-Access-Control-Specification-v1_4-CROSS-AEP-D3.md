...
### D3-RBAC-01 — Authorization boundary
RBAC determines whether an actor may perform a Session action. It does not own Session lifecycle/evidence state.

### D3-RBAC-02 — Capability categories
Subject to final permission governance:
- session view/discovery;
- session create/update/cancel;
- session schedule/manage;
- enrollment/access management;
- provider binding management;
- evidence/attendance review;
- completion evaluation/override where authorized;
- recording/artifact management;
- audit/provenance access.

These are capability categories, not final permission IDs.

### D3-RBAC-03 — Host / Instructor hold
Do not infer `Instructor = Host`. Final Host/Instructor authorization taxonomy remains dependent on MADCR-053/054.

### D3-RBAC-04 — Boundary preservation
Session access requires applicable authorization plus ACTIVE enrollment. Commercial Entitlement is not itself a role. Payment success does not mutate RBAC.

### D3-RBAC-05 — Physical hold
Exact permission IDs, role assignments, scopes and RLS remain downstream.
