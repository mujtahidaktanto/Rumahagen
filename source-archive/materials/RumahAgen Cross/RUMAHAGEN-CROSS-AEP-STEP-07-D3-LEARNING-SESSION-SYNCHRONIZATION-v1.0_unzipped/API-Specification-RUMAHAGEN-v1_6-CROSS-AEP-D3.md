...
### D3-API-01 — Learning Session semantic contract
Required capability families:
- Session discovery/read;
- Session lifecycle management;
- Session Enrollment create/read/state transition;
- session access resolution;
- Provider Binding management by authorized actors;
- provider event/evidence ingestion;
- attendance/completion evaluation outcome access;
- optional artifact/recording reference;
- audit/provenance access as authorized.

### D3-API-02 — Canonical boundaries
- `Learning Session ID ≠ Provider Session ID`.
- Course enrollment endpoints remain M04.
- Event RSVP endpoints remain M05.
- Provider callback/webhook receipt is evidence input, not completion.
- Session completion cannot directly create LP, Credential or Award Instance.
- Paid access consumes Commercial outcome; Learning Session does not implement payment logic.

### D3-API-03 — Enrollment access
Session Enrollment uses:
`PENDING → ACTIVE → COMPLETED`.

Activation consumes applicable entitlement/authorization outcome. Payment success alone is not the semantic definition of ACTIVE; free access may activate without payment.

### D3-API-04 — Evidence
Normalized/idempotent Participation Evidence precedes Attendance Evaluation and Completion Policy.

### D3-API-05 — Open/deferred API details
Exact endpoint paths, payloads, status enums, webhook/event contracts, provider OAuth scopes, retry/backoff and exact permission IDs remain downstream.
