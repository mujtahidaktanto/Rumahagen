...
**Status:** Synchronized semantic architecture layer; physical schema/API implementation remains downstream.

## 25A. Cross-AEP Authority Overlay
The architecture now explicitly recognizes four cooperating authority domains:

| Domain | Primary authority |
|---|---|
| Commercial / Payment | Subscription, Payment Core, verification, fulfillment, Commercial Entitlement and quota-capacity authority |
| Learning Economy | Learning progression, Learning Point ledger, learning/credential progression |
| Learning Session | Session lifecycle, Enrollment, Provider Binding, participation evidence and attendance/completion policy boundary |
| Title / Awarding | Title identity, Awarding Path Version, Awarding Rule Version and Award Instance |

No domain may silently write another domain's authoritative state.

## 25B. D1 Commercial / Payment Architecture
The Commercial/Payment boundary is now canonicalized as:
...
- Payment Core is provider-independent.
- Provider-specific mechanics remain behind the Provider Adapter boundary.
- Provider callback/webhook receipt is evidence/input, not confirmed payment.
- Verification is mandatory before confirmed payment.
- Idempotency is mandatory before fulfillment.
- Confirmed Payment is the only payment-state input that may cross into commercial fulfillment.
- Commercial Entitlement is distinct from Subscription and RBAC.
- Payment success does not directly mutate RBAC.
- Reconciliation does not itself authorize commercial mutation.
- Historical purchase terms remain an immutable commercial-complete snapshot after confirmation.
- Configurable commercial parameters must not rewrite historical purchases or grants.

## 25C. Cross-domain handoff
Commercial provides trusted payment/access outcomes to downstream domains. Learning Session may consume commercial access/entitlement outcomes, but remains the authority for Session lifecycle. Learning Economy owns Learning Point processing. Awarding owns Award Instance creation and historical award provenance.

## 25D. Deferred physical layer
No exact physical table, column, API endpoint, webhook implementation, RBAC ID, provider credential model, idempotency table, migration or state enum is introduced by this addendum. Those require the later D1 downstream technical synchronization and any still-open governance decisions.
