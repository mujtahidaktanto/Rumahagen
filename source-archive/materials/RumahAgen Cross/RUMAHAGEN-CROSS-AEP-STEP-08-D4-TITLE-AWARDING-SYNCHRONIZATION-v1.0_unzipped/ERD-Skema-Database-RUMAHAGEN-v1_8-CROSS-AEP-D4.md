...
### D4-ERD-01 — Canonical Awarding logical domain
The downstream model must represent, as distinct logical concepts:
- Title Definition / stable Title Identity;
- Awarding Path;
...
- Appeal / decision history where applicable;
- Presentation Preference;
- Authority / Scope Binding;
- provenance/audit.

### D4-ERD-02 — Canonical provenance
`Title Identity → Awarding Path Version → Awarding Rule Version → Qualification → Award Instance`.

A Title Identity Version entity is NOT introduced.

Award Instance must retain historical Path Version and Rule Version semantics. Used versions are semantically immutable; superseded used versions are not reactivated.

### D4-ERD-03 — Critical separations
- Title Definition ≠ Award Instance.
- Awarding Path Version ≠ Awarding Rule Version.
- Qualification ≠ Award Instance.
- Certificate/Credential ≠ Award Instance.
- Presentation Preference ≠ Award ownership/lifecycle.
- Learning/Partner outcome ≠ Award authority.
- Commercial Payment/Entitlement ≠ Award authority.

### D4-ERD-04 — Presentation
Primary selection: at most 1.
Featured selection: at most 3.
Removing a presentation selection must not delete/mutate the Award Instance.
Revoked Awards must not remain Featured; expired Awards may remain Featured where policy permits.

### D4-ERD-05 — Historical integrity
Award identity survives Expired, Revoked, Appeal and Restored states. Successful restoration returns the same Award Instance; it does not create a new Award.

### D4-ERD-06 — Controlled open items
Do not lock:
- Authority/Scope cardinality (AEP3-OD-02);
- lifecycle/appeal storage mechanism (OD-03);
- Rule lineage cardinality (OD-04);
- Rule temporal/effective-date semantics (OD-05);
- repeat/renewal uniqueness where business policy is not explicit.
No physical migration is authorized by this step.
