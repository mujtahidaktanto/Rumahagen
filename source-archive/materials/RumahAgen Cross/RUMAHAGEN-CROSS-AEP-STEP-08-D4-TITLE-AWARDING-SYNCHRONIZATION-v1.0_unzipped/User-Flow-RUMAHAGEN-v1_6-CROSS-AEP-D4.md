...
### D4-FLOW-01 — Canonical earning journey
`Title / Path defined → Agent enters qualifying route → Prerequisites → Evidence/Learning/Assessment → Qualification Evaluation → Qualified? → Award Instance → Lifecycle → Presentation`.

If not qualified, no Award Instance is created.

### D4-FLOW-02 — Multiple paths
A Title may have multiple qualifying paths. Satisfying multiple routes must preserve provenance without creating duplicate ownership Awards solely because more than one route was satisfied.

### D4-FLOW-03 — Award presentation
`Earned Titles → Select Primary (0–1) → Select Featured (0–3) → Order Featured → Save preference`.
Presentation changes never delete Award ownership.

### D4-FLOW-04 — Revocation/appeal
`Award Revoked → Appeal window from revocation timestamp → Appeal → Authority review → Decision → Restore same Award OR remain Revoked`.

### D4-FLOW-05 — Cross-domain evidence
Learning completion, assessment, LP, credential and partner outcomes are evidence sources. Awarding remains qualification and Award Instance authority. Payment confirmation does not directly create an Award.

### D4-FLOW-06 — Authorization
Privileged Awarding operations require effective authorization (role + capability + authority/scope). Qualification remains a separate server-side evaluation.

### D4-FLOW-07 — Open dependency handling
OD-02–05 are not represented as resolved behavior in the user flow; the flow remains semantic where those details are physical/implementation dependent.
