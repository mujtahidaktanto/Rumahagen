...
### D4-DICT-01 — Dictionary follows Awarding logical model
The physical dictionary must eventually cover Title, Path, Path Version, Rule, Rule Version, Qualification Evaluation, Evidence, Award Instance, Lifecycle History, Appeal, Presentation Preference, Authority/Scope Binding and provenance.

### D4-DICT-02 — Historical provenance
Historical Award interpretation must not depend only on mutable current Title configuration. Used Path/Rule versions must remain addressable historically.

### D4-DICT-03 — M04 isolation
Existing M04 Certificate/Credential storage remains M04. Certificate rows must not be migrated into or reused as Award Instance storage.

### D4-DICT-04 — Physical hold
No final table/column/FK/cardinality/index/state implementation is introduced. OD-02–05 remain explicit where physical design depends on them.
