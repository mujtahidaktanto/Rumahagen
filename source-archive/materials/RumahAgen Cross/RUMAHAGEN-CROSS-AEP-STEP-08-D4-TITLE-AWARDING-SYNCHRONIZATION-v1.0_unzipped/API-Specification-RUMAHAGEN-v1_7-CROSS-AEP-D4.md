...
**Versi:** 1.3 (naik dari 1.2 — kontrak deteksi duplikat foto di `POST /listings/{id}/media` dan `PATCH /listings/{id}/status`, `ADR-047`/`OD-25`; ditambah pemulihan 4 endpoint `/admin/internal-users` OD-20 yang sempat hilang tanpa jejak resmi di file sumber v1.2 "Baseline"; ditambah pemulihan 3 endpoint `GET/PUT/DELETE /admin/developer-projects{/id}` yang diklaim sudah ada sejak audit v1.1/T4-11 [2026-08-06] namun terbukti tidak pernah ditambahkan — ditemukan lewat audit konsolidasi `MP-06-DirektoriDeveloper-Module-Planning.md`, 9 Agustus 2026)
**Tanggal:** 8 Agustus 2026 (kontrak duplikat foto) — dilengkapi pemulihan endpoint OD-20 pada 9 Agustus 2026 setelah audit konsolidasi versi menemukan endpoint tsb hilang tanpa jejak resmi antara v1.2 dan v1.2 "Baseline"; dilengkapi lagi pemulihan 3 endpoint `/admin/developer-projects{/id}` pada 9 Agustus 2026 (regresi ketiga yang ditemukan pada siklus konsolidasi module planning yang sama — lihat §10.3); tidak ada endpoint lain yang diubah pada ketiga siklus ini
...
| GET | `/admin/developer-projects` *(baru, 9 Agustus 2026 — lihat catatan di bawah)* | Superadmin, Manager, Admin |
| PUT | `/admin/developer-projects/{id}` *(baru, 9 Agustus 2026)* | Superadmin, Manager, Admin |
| DELETE | `/admin/developer-projects/{id}` *(baru, 9 Agustus 2026)* | Superadmin, Manager, Admin |

> **(9 Agustus 2026, koreksi regresi):** `MP-06-DirektoriDeveloper-Module-Planning.md` §51 Konflik #2 mencatat 3 endpoint ini sebagai **"✅ Diperbaiki [2026-08-06], audit v1.1/T4-11"**, namun verifikasi silang siklus konsolidasi 9 Agustus 2026 membuktikan endpoint tsb **tidak pernah benar-benar ditambahkan** ke file ini — pola regresi dokumentasi-vs-implementasi yang sama juga ditemukan di `0009_m04_learning_center.sql` dan `0010_m05_events.sql`. Ditambahkan sekarang atas instruksi Owner, melengkapi CRUD penuh sesuai Functional Specification §4.6 ("CRUD data developer partner & proyek").
...
### D4-API-01 — Canonical Awarding resource boundary
The downstream API contract must expose semantic capabilities for:
- Title catalog/read/manage;
- Awarding Path and Path Version;
- Awarding Rule and Rule Version;
- Qualification evaluation;
- Evidence submission/review;
- Award Instance read/issue/lifecycle;
- Appeal;
- Presentation preference;
- provenance/audit.

### D4-API-02 — Qualification and Award separation
Qualification is evaluated server-side against the applicable Path/Rule Version. A successful qualification may create a governed Award Instance; a learning completion, certificate, LP balance or payment confirmation alone does not directly create an Award unless the configured Awarding Rule explicitly makes it qualifying evidence.

### D4-API-03 — Historical lifecycle
Revocation, appeal, restoration and expiration are explicit governed actions. Successful appeal restores the same Award Instance. Generic arbitrary status mutation is not the canonical contract.

### D4-API-04 — Presentation
Primary ≤1 and Featured ≤3 per applicable presentation scope. Presentation changes do not mutate Award ownership/provenance.

### D4-API-05 — Authorization
Effective Awarding authorization is:
`Existing Actor Role + Capability Permission + Authority/Scope Binding`.
Authorization ≠ qualification.

### D4-API-06 — Retry safety
Qualification/issuance retries must not create unintended duplicate Awards where repeat/renewal policy disallows duplication. Appeal retry must not create unintended multiple active appeals.

### D4-API-07 — Physical/API hold
Exact endpoint paths, payloads, HTTP mappings, permission IDs, RLS rules and physical implementation remain downstream.
