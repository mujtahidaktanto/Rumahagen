...
-- ============================================================================
-- CHANGELOG:
-- [2026-08-10] Fix RLS org_invitations_insert (T1-04, Konflik #1 MP-12).
--   Dokumen MP-12 dan CHANGELOG-v0.7.1/v0.7.2 mencatat perbaikan ini sebagai
--   "✅ Diperbaiki [2026-08-06]" — namun verifikasi audit konsolidasi 10
--   Agustus 2026 membuktikan file migration ini (versi sebelum patch ini)
--   TIDAK PERNAH benar-benar diubah, masih WITH CHECK (agent_id=auth.uid()
--   OR leader_id=auth.uid()) tanpa verifikasi keanggotaan. Regresi ketujuh
--   dari pola yang sama dengan migration 0008/0009/0010 dan API Spec
--   §10.3/§11.3 — klaim "Diperbaiki" di dokumentasi tidak tercermin di file
--   sumber. org_invitations_insert sekarang benar-benar kondisional per
--   initiated_by_type: 'agent_request' cukup agent_id sendiri, 'leader_invite'
--   wajib EXISTS check keanggotaan Leader aktif — menutup celah spoofing
--   undangan palsu atas nama Organization manapun.
-- ============================================================================
...
-- WITH CHECK kondisional per initiated_by_type (T1-04, Konflik #1 MP-12):
-- 'agent_request' cukup agent_id=auth.uid() (agen mengajukan diri sendiri);
-- 'leader_invite' WAJIB tambahan verifikasi bahwa leader_id yang diklaim benar-benar
-- Leader aktif dari organization_id yang direferensikan — mencegah spoofing undangan
-- palsu atas nama Organization yang bukan miliknya.
CREATE POLICY org_invitations_insert ON organization_invitations FOR INSERT TO authenticated
  WITH CHECK (
    (initiated_by_type = 'agent_request' AND agent_id = auth.uid())
    OR (initiated_by_type = 'leader_invite' AND leader_id = auth.uid()
