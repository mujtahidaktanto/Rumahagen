...
export type RefreshState = "siap" | "sudah_hari_ini" | "kuota_habis" | "tidak_tersedia" | "tidak_diketahui";
...
export function refreshState(l: { status: string; lastRefreshedAt: string | null }, quota: RefreshQuota | null | "gagal", now: Date = new Date()): RefreshState {
  ...
  if (quota === "gagal" || quota === null) return "tidak_diketahui"; // tanpa kolam kuota terbaca: tombol tetap boleh dicoba, server memutuskan
  return quota.usedToday >= quota.allowance ? "kuota_habis" : "siap";
}
