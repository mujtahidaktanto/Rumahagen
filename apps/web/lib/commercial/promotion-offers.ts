...
// alasan bila tidak, dan harga akhir; tanpa membuka konfigurasi promosi. organizationId (opsional, 0145) menentukan cakupan pembelian: kosong = pribadi;
// aturan promosi applies_to dievaluasi terhadap cakupan ini. Pengguna anonim mendapat promotion_offer null
...
export async function annotateAddonsWithPromotionOffers<T extends { id: string; promotion_id?: string | null }>(
  ...
  if (userId && withPromo.length > 0) {
    const { data, error } = await supabase.rpc("my_addon_promotion_offers", { p_addon_ids: withPromo.map((a) => a.id), p_organization_id: organizationId });
    if (error) {
      throw error;
    }
