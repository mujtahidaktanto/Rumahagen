...
// Aturan bentuk promosi ditegakkan juga oleh CHECK migration 0133. Respons memuat effective_status/is_applicable/benefit_label.
...
export const GET = withApiHandler({}, async (ctx) => {
  ...
  let query = supabase
    .from("promotions")
    .select("id, code, name, status, valid_from, valid_to, benefit_configuration, created_at, updated_at", { count: "exact" })
  ...
  const now = new Date();
  const rows = (data ?? []).map((row) => ({ ...row, ...derivePromotionState(row, now) }));
