...
## `0146` — Tutup celah RPC anon dan pemalsuan audit (✅ DITERAPKAN 2026-09-25, setelah kode ter-deploy ke staging)
...
- **Hasil setelah diterapkan (Advisor):** fungsi SECURITY DEFINER yang bisa dipanggil anon 67 -> 16 (10 pembantu RLS, `verify_certificate`, `get_shared_dbr_simulation`, dan 4 pembantu internal yang belum ditutup: `new_certificate_verification_code`, `next_certificate_number`, `session_enrollment_agent`, `course_enroll_problem`); untuk authenticated 96 -> 60; `function_search_path_mutable` 12 -> 0. Uji langsung via REST dengan kunci anon: `create_notification`, `log_audit_event`, `check_and_increment_rate_limit` = 401; `verify_certificate` = 200. Sisa: view `public_agent_profiles` (hak berlebih), 60 fungsi authenticated (audit satu per satu, terutama `notify_expiring_listings`, `send_event_reminders`, `merge_listings`), leaked password protection (dashboard Supabase).
...
## `0150` — MVP M04: sertifikat terverifikasi, pengaturan belajar, aturan kuis pihak ketiga, LP awal (✅ DITERAPKAN 2026-09-25)
- Keputusan produk 2026-09-25 (lihat memory `project-m04-certificate-mvp`). `learning_settings` (satu baris, kolom eksplisit + CHECK, `version`, audit `m04.learning_settings.update`; RLS memakai permission `m04.learning_economy_configuration.view/.manage` yang sudah ada): batas/jeda kuis pihak ketiga (default jeda 60 menit, tanpa batas), penerbitan sertifikat otomatis, template dan penandatangan bawaan (Mujtahid Aktanto, CEO RumahAgen, tanda tangan kosong), bonus LP akun baru 25, hadiah LP per kejadian (enroll, selesai, lulus kuis; default 0).
- `courses` menambah `organizer_type` (rumahagen|partner|instructor — penyelenggara, BUKAN provider live yang tetap di `session_provider_bindings.provider_key`), `certificate_template`, `signer_name/title/signature_path`, `partner_logo_paths` (maks 2), `quiz_max_attempts`, `quiz_cooldown_minutes`. Semua hanya bisa diubah staf (trigger; instruktur selalu `instructor`, tidak bisa mengubahnya); file harus di folder `courses/{id}/`. Trigger publish kini juga mewajibkan minimal 1 kuis (saat diajukan dan diterbitkan).
- `certificates` menambah `enrollment_id`, `certificate_number` (`RA-{tahun}-{6 digit}`, penghitung atomik per tahun `certificate_counters`), `verification_code` (`XXXX-XXXX-XXXX`, heks acak), `status` issued|revoked, kolom cabut, `snapshot` jsonb (nama pemegang, judul, template, penandatangan, logo saat terbit). Unik per (agent, kursus). Tabel tidak bisa ditulis langsung; fungsi: `issue_certificate_internal` (hanya untuk enrollment selesai, idempoten), `ensure_my_certificate(kursus)` (Agent; terbit ulang/backfill saat unduh), `admin_issue_certificate`, `revoke_certificate`, `verify_certificate(kode)` (anon; hanya kolom aman). Terbit otomatis saat enrollment selesai (bisa dimatikan; kegagalan tidak menggagalkan kelulusan). Blok backfill disertakan (tabel kosong saat ditulis).
- Aturan kuis: kursus `rumahagen` tanpa batas/jeda; `partner`/`instructor` memakai jeda dan batas (kursus menimpa global), ditegakkan trigger `quiz_attempts` dengan kunci advisory; `quiz_attempt_status(enrollment, kuis)` untuk UI.
- LP: `grant_learning_reward` internal (idempoten lewat `idempotency_key`), bonus awal di `handle_auth_user_sync` (sekali per akun; TIDAK diberikan surut ke akun lama), hadiah saat enroll/selesai/lulus kuis. Bucket privat `certificate-assets` (PNG/JPEG/WebP, 1 MB).
- API: `GET/PATCH /admin/learning/settings`, `POST /admin/learning/settings/signature/upload-url`, `GET/PUT /courses/{id}/certificate-config`, `POST /courses/{id}/certificate-assets/upload-url`, `GET /courses/{id}/certificate` (PDF Agent; menerbitkan bila belum ada), `GET /certificates/{id}/pdf`, `GET /certificates/verify/{kode}` (publik), `POST /admin/certificates` (sekarang lewat fungsi, hanya enrollment selesai), `POST /admin/certificates/{id}/revoke`, `GET /quizzes/{id}/attempt-status`, `GET /agents/me/certificates` diperluas. PDF: 4 template (classic, modern, corporate lingkaran, premium art deco) via pdf-lib + `qrcode` + `sharp` (dependensi baru di package.json); QR ke `{APP_BASE_URL}/verifikasi/{kode}` (halaman web verifikasi belum ada; API JSON sudah).
- Diuji rollback live (2 putaran): bonus 25 LP sekali; publish tanpa kuis 23514, dengan kuis OK; staf ubah settings (version naik, audit tercatat), CHECK LP negatif ditolak, instruktur tidak bisa ubah settings; organizer dipaksa `instructor`, instruktur tidak bisa ubah organizer/signer (42501); logo/tanda tangan di folder lain dan 3 logo ditolak; jeda langsung 23514 (60 menit), batas 2 percobaan, internal 5 percobaan beruntun OK; sertifikat otomatis RA-2026-000001 + kode heks + snapshot (template modern, penandatangan kursus, logo), LP 25+2+5+10=42; otomatis mati lalu `ensure_my_certificate` menerbitkan 000002 (template/penandatangan bawaan), idempoten, kursus belum selesai 23514; Agent tidak bisa insert/update langsung atau mencabut (42501) dan hanya melihat miliknya; status kuis milik orang lain 42501; cabut, cabut dua kali 23514; verifikasi anon (huruf kecil) tampil status dicabut, kode salah 0 baris, anon tidak bisa membaca tabel; reward idempoten; bucket privat; nomor berurutan. Generator PDF diuji terpisah (4 template dirender dan dilihat; nama berisi emoji diganti `?`). tsc bersih. Setelah diterapkan diuji HTTP end-to-end (server dev lokal + Supabase live, 54 pemeriksaan, data uji dibersihkan): pengaturan, unggah logo/tanda tangan ke signed URL (PDF ditolak storage), config kursus (path folder lain dan file belum diunggah ditolak 422), jeda kuis 409, sertifikat otomatis + LP 42, unduh PDF (pemilik, unduh ulang, terbit ulang saat otomatis mati), verifikasi publik tanpa kebocoran data, cabut dan efeknya, 401/403/409 yang relevan. Catatan: validasi Zod mengembalikan 422 (konvensi proyek), bukan 400.
- Catatan: notifikasi sertifikat dilewati bila penerbit = pemilik (perilaku notify_user; unduh oleh Agent sendiri tidak memberi notifikasi). Aset lama tidak dihapus saat diganti karena snapshot sertifikat lama menunjuk path-nya. "Lewati lesson dengan LP" ditunda (butuh tabel progres lesson).

## `0151` — Notifikasi undangan/permohonan organisasi + sertifikat dicabut (M08/M12/M04) (✅ DITERAPKAN 2026-09-25)
- Temuan audit kotak masuk Agent (2026-09-25): undangan organisasi tidak memicu notifikasi apa pun. Trigger baru `trg_notify_org_invitation` pada `organization_invitations`: INSERT `leader_invite` -> Agent yang diundang ("Undangan bergabung organisasi"); INSERT `agent_request` -> leader ("Permohonan bergabung baru"); pending -> accepted/rejected -> pihak yang memulai (leader untuk undangan, Agent untuk permohonan); undangan leader dibatalkan -> Agent. Tipe `lainnya`, entity `organization_invitation`.
- `revoke_certificate()` (0150) kini memberi tahu pemilik: "Sertifikat dicabut" (tipe `lainnya`, entity `certificate`). notify_user melewati notifikasi untuk diri sendiri dan menelan galat, jadi tidak pernah menggagalkan penulisan.
- Diuji rollback live: undangan -> Agent, leader tidak dinotifikasi untuk undangannya sendiri, diterima -> leader (+ membership dibuat), dibatalkan -> Agent, permohonan -> leader, ditolak -> Agent, cabut sertifikat -> pemilik (staf pencabut tidak dinotifikasi). Tabel undangan kosong saat ditulis.
- Wireframe menyertai: kartu "Undangan untuk Anda" di M12-Organisasi-Dashboard (keadaan belum tergabung; terima/tolak, kedaluwarsa, memproses, gagal) dan contoh notifikasi baru + komentar diperbarui di M08-Pusat-Notifikasi.

## `0152` — Ketergantungan alamat provinsi > kota/kabupaten > kecamatan + data wilayah Kepmendagri 2025 (✅ DITERAPKAN 2026-09-25)
- Permintaan pemilik produk: kota/kabupaten harus milik provinsi yang dipilih; kecamatan harus milik kota/kabupaten yang dipilih (mis. Provinsi Jawa Timur + Kota Bandung ditolak). Sebelumnya `listings`, `developer_projects`, `agent_profiles` hanya punya FK terpisah ke `ref_*`.
- Fungsi `enforce_region_hierarchy()` (bukan SECURITY DEFINER; `ref_*` publik) + trigger `trg_region_hierarchy` BEFORE INSERT/UPDATE OF kolom wilayah pada ketiga tabel. Pelanggaran ERRCODE 23514 (API: 409). `agent_profiles` hanya punya province_id dan city_id; fungsi membaca district_id lewat `to_jsonb(NEW)`.
- Diuji rollback live: Jatim + Kota Bandung ditolak, kecamatan milik kota lain ditolak, ganti provinsi saja dengan kota lama ditolak, kombinasi benar lolos, update kolom lain tidak memicu. Logika di `agent_profiles` dan `developer_projects` memakai fungsi yang sama tetapi tidak diuji langsung (tabel kosong).
- **Data wilayah (bukan bagian file migration, terlalu besar):** `supabase/seed/wilayah/` (CSV bersih + `load.mjs` + `load.sql` + README). Dimuat 2026-09-25: 38 provinsi, 514 kab/kota (98 kota, 416 kabupaten; nama resmi lengkap, `Kota Bandung` dan `Kabupaten Bandung` terpisah), 7.285 kecamatan, 83.762 desa/kelurahan (65.779 berkode pos; kode pos opsional). Kode tanpa titik (2/4/6/10 digit). Kolom "METODE PENCOCOKAN" di sumber tidak dipakai.
- **Perbedaan kode lama:** seed lama memakai kode BPS (3171 = Jakarta Selatan); Kepmendagri memakai 3171 = Jakarta Pusat, 3174 = Jakarta Selatan. Upsert per `code` mengubah nama baris lama; listing uji "Test Listing Tier2" dipindahkan ke Jakarta Selatan / Kebayoran Baru (317407) agar alamatnya tetap sama.
- API baru (GET publik, filter berjenjang, `q` cari nama): `/ref-provinces`, `/ref-cities?province_id=`, `/ref-districts?city_id=`; desa lewat `/ref-villages?district_id=` (sudah ada). Filter bukan uuid dijawab 422.

## `0153` — Lanjutan Fase 0 keamanan: pembantu internal, hak view publik, current_role_code NULL-aman (✅ DITERAPKAN 2026-09-25)
- Hasil audit 44 fungsi SECURITY DEFINER yang bisa dipanggil pengguna login (dipanggil sebagai pengguna login tanpa hak staf, argumen acak): 27 menolak (42501/P0001/23xxx), 17 "jalan". Dari yang jalan: 8 pembantu baca murni (`addon_has_orders`, `promotion_in_use`, `quiz_has_attempts`, `quiz_problems`, `order_amount_currency`, `course_is_under_review`, `course_enroll_problem`, `session_enrollment_agent`; dipakai API/policy/trigger, dibiarkan), 3 tak sensitif (`refresh_listing` mengembalikan `listing_not_found`, `my_addon_promotion_offers`, `my_plan_promotion_offers`), dan 6 temuan nyata di bawah (4 guard NULL-tidak-aman + 2 pembakar nomor sertifikat).
- **Temuan 1 — guard NULL-tidak-aman:** `IF NOT (is_superadmin() OR current_role_code() IN (...)) THEN RAISE` tidak menolak pengguna login **tanpa baris `users`** (yatim), karena `NOT NULL = NULL`. Terkena: `admin_analytics_flow`, `admin_analytics_funnel`, `notify_expiring_listings`, `send_event_reminders` (RPC) dan dua trigger. `users.role_id` NOT NULL dan tiap akun auth mendapat baris `users`, jadi risiko nyata rendah, tetapi ditutup: `current_role_code()` kini `COALESCE(..., '')` (tidak ada kode/policy yang membandingkannya dengan NULL; dicek di pg_proc, pg_policy, apps/web).
- **Temuan 2 — pembakaran nomor sertifikat:** `next_certificate_number()` (menulis penghitung) dan `new_certificate_verification_code()` bisa dipanggil pengguna login lewat RPC. Dicabut dari anon dan authenticated (hanya dipanggil `issue_certificate_internal`, SECURITY DEFINER). `course_enroll_problem` dan `session_enrollment_agent` dipanggil trigger INVOKER: hanya anon yang dicabut.
- **Temuan 3 — hak berlebih view:** `public_agent_profiles` memberi anon dan authenticated semua hak; dicabut, disisakan SELECT (tetap SECURITY DEFINER sebagai proyeksi publik yang disengaja).
- Diuji rollback live: pengguna yatim ditolak di keempat RPC; anon dan authenticated ditolak di kedua pembantu sertifikat; rantai SECURITY DEFINER tetap bisa memanggilnya; `course_enroll_problem` tetap jalan untuk authenticated; anon bisa SELECT view. Tidak diuji langsung: penerbitan sertifikat penuh (butuh enrollment selesai) dan pemblokiran DELETE view oleh hak (view tak bisa ditulis sehingga galatnya 55000 sebelum cek hak; hak diverifikasi lewat information_schema setelah diterapkan).
...
## `0155` — Title otomatis dari kelulusan course (✅ DITERAPKAN 2026-09-26)
- Kolom baru `courses.awards_title_definition_id` (FK title_definitions). Staf (m04.course.publish) menghubungkan SATU title ke course; saat Agent menyelesaikan course (`enrollments.status = completed`, otomatis saat semua kuis lulus) sistem mencatat `qualification_evaluations` qualified (system_automated, provenance course_completion) lalu `award_instances` active. Trigger M15 lama (rebalance judul utama, notifikasi) ikut bekerja.
- Aturan: title harus active dan punya `title_authority_scopes` aktif saat dihubungkan; idempoten dan hormat pencabutan (sudah punya award apa pun statusnya -> tidak diberi lagi); kegagalan pemberian title hanya WARNING (kelulusan course tidak pernah gagal); menghubungkan ke course yang sudah punya lulusan memberi title ke lulusan yang belum punya. `enforce_course_certificate_config_staff_only` diperluas (kolom baru staf saja).
- Diuji rollback di DB live: backfill tidak menggandakan award manual tester1; agen 2 lulus -> award + evaluasi + judul utama + 3 notifikasi otomatis; panggil ulang dan setelah dicabut tidak diberi lagi; title tanpa scope ditolak (23514); instruktur pemilik course ditolak mengubah title (42501). Setelah diterapkan: perluas `courseCertificateConfigSchema`/rute `certificate-config` (awards_title_definition_id) dan tampilkan "Title yang didapat" di Detail Course.

## `0156` — `organization_id` di view `public_agent_profiles` (✅ DITERAPKAN 2026-09-26)
- Keputusan 2026-09-26: anggota tim halaman publik Organisasi dicocokkan dengan ID organisasi, bukan nama (nama tidak unik). `CREATE OR REPLACE VIEW` menambah kolom `organization_id` (= `org.id` dari join organisasi aktif; NULL bila tanpa organisasi/tidak aktif) di AKHIR view; kolom lama, urutan, dan hak (anon/authenticated SELECT saja, 0153) tidak berubah. ID organisasi bukan data sensitif (sudah publik lewat `organizations_select_active_public`). Indeks baru `idx_agent_profiles_organization_id`.
- Diuji rollback di DB live sebagai `anon`: anggota per ID = 2, kolom lama utuh (title utama tetap terbaca), hak tetap. Setelah diterapkan: `lib/public/organization-data.ts` mengganti `.eq("organization_name", ...)` menjadi `.eq("organization_id", org.id)` dan menghapus catatan nama-tidak-unik di FRONTEND_GAPS.

## `0157` — Alamat profil publik Agent bisa diganti (✅ DITERAPKAN 2026-09-26)
- Keputusan 2026-09-26: Agent boleh mengganti `agent_profiles.public_slug` (alamat `/agen/{slug}`) sendiri, dengan pengecekan ketersediaan dan batas 1 kali per bulan kalender WIB (reset tanggal 1). Kolom baru `slug_changed_at` (diisi trigger; tidak bisa diatur klien). Fungsi `is_reserved_agent_slug`, `agent_slug_problem` (format huruf kecil/angka/tanda hubung, 3-60 karakter, kata dicadangkan, dipakai profil lain, atau alamat lama profil lain di `url_redirects`), `next_wib_month_start`, trigger `enforce_agent_slug_change` (BEFORE UPDATE OF public_slug, slug_changed_at), RPC `check_my_agent_slug(p_slug)` (untuk authenticated).
- **Perbaikan**: trigger pengalihan `trg_agent_slug_redirect` memakai awalan `/agent/` (area aplikasi) padahal alamat publik profil `/agen/`; diganti `/agen/`.
- Diuji rollback di DB live sebagai `authenticated` (tester1 dan shalovaid1): cek baru/dipakai/dicadangkan/format/panjang/sama benar; ganti pertama berhasil + pengalihan `/agen/lama -> /agen/baru`; penggantian kedua di bulan sama ditolak (pesan memuat tanggal boleh lagi); klien mengosongkan `slug_changed_at` tak berpengaruh; alamat lama orang lain, slug aktif orang lain, dan kata dicadangkan ditolak; bulan berikutnya boleh ganti kembali ke alamat lama sendiri. Temuan uji: `agent_slug_problem` perlu `GRANT EXECUTE` ke authenticated karena trigger berjalan sebagai pemanggil.
- **URUTAN DEPLOY** (sudah dipenuhi: 0157 diterapkan sebelum kode dideploy). Halaman Profil Saya membaca kolom `slug_changed_at` (galat bila belum ada) dan `PUT /users/profile` menerima `public_slug` (tanpa trigger, batas dan aturan tidak berlaku).

## `0158` — Bucket foto profil dan foto listing (✅ DITERAPKAN 2026-09-26)

- Dua bucket **publik-baca**, batas 3 MB per berkas, hanya `image/webp` dan `image/jpeg`: `avatars` ("{user_id}/{uuid}-512.webp") dan `listing-photos` ("{user_id}/{listing_id}/{uuid}-{400|1080|2048}.webp"). Tidak ada perubahan tabel dan tidak ada policy pada `storage.objects`: penulisan hanya lewat signed upload URL buatan server (service role setelah kepemilikan diperiksa), sama seperti bucket `agent-ktp` (0149).
- Keputusan pemilik produk: pilih berkas di browser hingga 25 MB, browser mengecilkan (WebP, cadangan JPEG) dan menyimpan tiga varian ukuran web; `listing_photos.url` menyimpan URL varian 2048, varian 400/1080 diturunkan dari nama berkas (`lib/media/variants.ts`).
- API baru: `POST /agents/me/avatar/upload-url`, `PUT|DELETE /agents/me/avatar`, `POST /listings/{id}/media/upload-url`; `POST /listings/{id}/media` memeriksa foto hasil unggah milik pemanggil dan lengkap tiga varian; `DELETE` media ikut menghapus berkas; `PUT /users/profile` menolak `avatar_url` bebas.
- **URUTAN DEPLOY:** sudah dipenuhi, 0158 diterapkan sebelum kode dideploy (tanpa bucket, unggah gagal). Rollback: hapus kedua bucket bila kosong.

## `0159` — Jatah Refresh Listing: gratis harian + bonus per orang + saldo add-on (✅ DITERAPKAN 2026-09-26)

- **Masalah:** hanya Agent yang punya "kolam kuota" refresh yang bisa refresh, dan di DB live 0 Agent punya kolam (tidak ada pemberian otomatis), sehingga semua refresh ditolak dengan pesan menyesatkan "kuota habis". Kolam dari add-on juga punya `daily_refresh_allowance` NULL sehingga `used >= NULL` tidak pernah benar (tak terbatas).
- **Keputusan pemilik produk (2026-09-26):** (1) SEMUA Agent otomatis punya jatah **gratis harian** (bawaan 5, reset 00.00 WIB); Superadmin mengubah angka bawaan dan memberi **bonus harian per orang** (mis. hadiah ulang tahun +10, masa berlaku opsional). (2) Paket refresh dari **add-on (pembelian) = saldo**: tanpa masa berlaku, tanpa reset, tersimpan sampai habis. (3) **Urutan pemakaian: gratis, bonus, lalu saldo add-on.**
- **Aturan:** jatah harian efektif = `system_configs.refresh_allowance.default_daily` (seed 5, validasi 0-999 oleh trigger) + bonus aktif (entitlement `refresh_bonus`) + tambahan tetap hasil `configure_refresh_allowance` (entitlement `listing_refresh_allowance`), masing-masing sesuai masa berlakunya. Pemakaian harian dihitung per tanggal WIB. Saldo add-on = `granted_quantity` kolam `<kode>:listing_refresh` dikurangi seluruh pemakaian kolam itu sepanjang waktu (masa berlaku diabaikan; pembelian tertua dipakai lebih dulu, dikunci per Agent dengan advisory lock). `grant_addon_capacity` kini selalu menyimpan `ends_at`/`valid_to` NULL untuk `listing_refresh`. Kolam dasar (jatah 0) dibuat malas saat refresh pertama sebagai penampung catatan harian; tidak perlu mengisi Agent lama.
- **Fungsi:** baru `refresh_default_allowance`, `refresh_allowance_state`, `ensure_refresh_base_pool` (internal, dicabut dari anon/authenticated), `my_refresh_allowance` (Agent), `grant_refresh_bonus`, `revoke_refresh_bonus`, `admin_refresh_allowance` (Superadmin, penjaga di dalam). Diubah: `consume_refresh_allowance`, `refresh_listing` (alasan baru `agent_refresh_allowance_none` bila jatah harian 0 dan saldo 0; `remaining_today` = sisa harian + sisa saldo), `grant_addon_capacity`, `agent_statistics_summary` (blok quota; kunci lama tetap, ditambah `default_daily`, `extra_daily`, `stock_remaining`; `has_pool` = ada jatah harian atau saldo).
- **Uji rollback (2026-09-26):** Agent tanpa kolam berhasil refresh (bawaan 5); setelah harian habis, saldo add-on 3 terpakai (walau tanggal add-on sengaja sudah lewat); simulasi hari berikutnya: harian pulih ke 5 dan saldo tidak reset; urutan gratis dulu baru saldo (agen lain: percobaan ke-6 memakai add-on); bonus +10 -> 15 lalu dicabut -> 5; Agen biasa tidak bisa memberi bonus; nilai bawaan 1000 dan kunci asing ditolak; bawaan 0 tanpa saldo -> tidak ada jatah; `grant_addon_capacity` listing_refresh tanpa masa berlaku; anon ditolak.
- **API:** `GET /agents/me/refresh-allowance`, `GET /admin/agents/{id}/refresh-allowance`, `POST /admin/agents/{id}/refresh-bonus`, `DELETE /admin/refresh-bonus/{poolId}`; angka bawaan lewat `PUT /admin/config/system/refresh_allowance.default_daily`. Kartu Refresh di Detail Listing menampilkan jatah harian, saldo paket, dan keadaan "Belum Ada Jatah Refresh".
- **Urutan deploy:** kode kompatibel sebelum dan sesudah 0159 (kunci baru opsional); tanpa 0159 refresh tetap ditolak seperti sekarang. Rollback: kembalikan fungsi yang diubah ke 0020/0081/0125, DROP fungsi baru dan trigger, hapus baris config.

## `0160` — Integritas pendaftaran event + layar pendaftar penyelenggara (✅ DITERAPKAN 2026-09-26)

- **Celah yang ditutup (pemindaian 2026-09-26):** (1) peserta bisa mengubah status pendaftarannya sendiri (menyetujui diri sendiri, menandai hadir) karena trigger `enforce_event_registration_organizer_rules` meloloskan pemilik; (2) peserta bisa menyisipkan pendaftaran berstatus bebas, ke event belum tayang atau privat, dan mendaftar berkali-kali (tanpa indeks unik).
- **Aturan baru:** peserta hanya boleh membatalkan (registered|waitlist|pending_approval -> cancelled) dan tidak boleh mengubah kolom lain (pendaftaran tamu boleh mengganti email tamu); status awal harus `registered`; hanya event `published` dan bukan privat (kecuali penyelenggara); satu pendaftaran aktif per (event, Agent) mode self (indeks unik parsial `uq_event_registrations_active_self`; yang dibatalkan boleh mendaftar ulang). Pemilik yang juga penyelenggara memakai aturan penyelenggara. Staf (Superadmin, Admin, Manager) dan service_role tidak dibatasi.
- **RPC baru** `event_registrants(event)` untuk penyelenggara/staf (nama Agent diambil di dalam fungsi karena RLS `agent_profiles` tidak mengizinkan penyelenggara). Perbaikan Advisor: `search_path` tetap pada `is_reserved_agent_slug` dan `next_wib_month_start` (0157).
- **Uji rollback (2026-09-26):** pendaftaran normal -> registered; status awal attended, pendaftaran ganda, event pending/privat/closed ditolak; event manual -> pending_approval; peserta tidak bisa menyetujui diri, menandai hadir, atau mengganti agent; peserta bisa membatalkan lalu mendaftar ulang; peserta biasa dan anon tidak bisa memanggil `event_registrants`; penyelenggara melihat daftar (dengan nama), menyetujui, menandai hadir, tidak bisa membalik hadir atau mengganti agent; notifikasi persetujuan terkirim; peserta yang sudah hadir tidak bisa membatalkan; search_path terpasang.
- **API:** `DELETE /events/{id}/rsvp` (batalkan pendaftaran sendiri), `GET /events/{id}/registrations` (penyelenggara), `PATCH /events/{id}/registrations/{rid}` { status } (setujui/tolak/hadir); `POST /events/{id}/rsvp` kini memberi 409 berpesan untuk pendaftaran ganda dan event yang belum tayang. **UI:** tombol Batalkan di Event Saya, kartu Pendaftar di Kelola Event.
- **Urutan deploy:** sudah dipenuhi, 0160 diterapkan sebelum kode dideploy (tanpa 0160 tombol Batalkan dan aksi penyelenggara berjalan tetapi celah lama tetap terbuka; `event_registrants` tidak ada sehingga kartu Pendaftar menampilkan keadaan gagal). Rollback: kembalikan dua fungsi trigger ke definisi 0088/0129, DROP INDEX, DROP FUNCTION event_registrants, RESET search_path.

## `0161` — Integritas undangan organisasi + daftar anggota dan pemohon + bucket logo/banner (✅ DITERAPKAN 2026-09-26)

- **Celah serius yang ditutup (ditemukan saat membangun Fase 3f):** `organization_invitations_update` mengizinkan Agent memperbarui SEMUA kolom baris undangannya. Lewat REST langsung, Agent bisa mengganti `organization_id` ke organisasi mana pun atau mengubah `initiated_by_type` dari `agent_request` menjadi `leader_invite`, lalu `status = 'accepted'`: trigger keanggotaan membuat anggota tanpa persetujuan leader. Undangan yang sudah rejected/cancelled/kedaluwarsa juga masih bisa diterima, dan status awal saat INSERT bebas.
- **Aturan baru (trigger `enforce_organization_invitation_rules`, SECURITY DEFINER, non-staf):** kolom identitas dan kedaluwarsa tidak bisa diubah; hanya dari `pending`; undangan leader: yang diundang menerima/menolak, leader membatalkan; permohonan Agent: peminta membatalkan (menarik), leader menerima/menolak; menerima butuh belum kedaluwarsa dan organisasi `active`. INSERT: status `pending`, organisasi `active`, bukan anggota aktif, undangan atas nama leader pengirim, permohonan ke leader aktif organisasi itu. Indeks unik parsial `uq_org_invitation_pending` (satu pending per organisasi, Agent, jenis).
- **RPC (RLS `agent_profiles` tidak mengizinkan sesama anggota membaca profil):** `organization_roster` (anggota), `organization_pending_requests` (leader), `my_organization_invitations` (yang diundang, termasuk yang kedaluwarsa), `search_invitable_agents` (leader; hanya profil publik, bukan anggota, tanpa undangan pending).
- **Bucket publik-baca** `organization-media` (logo/banner, WebP/JPEG, 3 MB; penulisan lewat signed upload URL, pola 0158).
- **Uji rollback (2026-09-26):** roster bagi leader 2 anggota, non-anggota ditolak; undangan leader OK, status awal bebas/ganda/mengundang anggota/atas nama lain/organisasi tidak aktif ditolak; Agent melihat undangan dengan nama organisasi dan leader; ganti organisasi, ganti jenis, dan membatalkan sebagai yang diundang ditolak; menerima OK lalu keanggotaan terbentuk (menerima ulang = tanpa efek); permohonan: menerima sendiri, mengubah jenis lalu menerima, dan leader palsu ditolak, leader menerima OK (roster 4); dibatalkan lalu diterima ditolak; kedaluwarsa tampil "Kedaluwarsa" dan tidak bisa diterima; non-leader ditolak mencari Agent; anon ditolak; bucket ada. Satu bug rancangan ditemukan dan diperbaiki saat uji (pemeriksaan leader dijalankan dengan hak peminta yang tidak boleh membaca anggota) sehingga trigger dibuat SECURITY DEFINER.
- **API:** `PUT /organization-invitations/{id}/cancel`, `GET /organizations/{id}/invitable-agents?q=`, `POST /organizations/{id}/branding/upload-url`; `PUT .../branding` kini hanya menerima logo/banner hasil unggahan sendiri dan menghapus berkas lama; accept/reject/invitations/join-requests memberi 409/403 berpesan. **UI:** `/agent/organisasi` (undangan untuk Anda, dashboard, Edit Branding, Tutup Organisasi 2 langkah + OTP, Keluar), `/agent/organisasi/baru`, `/agent/organisasi/anggota`; kartu "Bergabung" di `/organisasi/[slug]`.
- **Urutan deploy:** sudah dipenuhi, 0161 diterapkan sebelum kode dideploy (tanpa 0161 daftar anggota/pemohon dan kartu undangan menampilkan keadaan gagal, unggah logo gagal, dan celah undangan tetap terbuka). Rollback: DROP TRIGGER trg_enforce_organization_invitation_rules + fungsinya, DROP INDEX uq_org_invitation_pending, DROP FUNCTION empat RPC, hapus bucket bila kosong.

## `0163` — Tautan berbagi DBR hanya mengembalikan bidang hasil (✅ DITERAPKAN 2026-09-26, diuji rollback sebelumnya)

- **Masalah:** `get_shared_dbr_simulation` (0089) mengembalikan seluruh baris `dbr_simulations` kepada pemegang token (termasuk `anon`): `agent_id`, `listing_id`, `bank_id`, `share_token`, `prospect_phone`, `net_income`, `existing_installments`, `pdf_export_url`.
- **Perbaikan:** dibuat ulang (DROP + CREATE, tipe kembalian berubah) sebagai `RETURNS TABLE` sempit: `bank_name, prospect_name, property_price, down_payment, loan_amount, tenor_months, interest_rate_annual, monthly_installment, dbr_percent, eligibility_status, threshold_used, created_at`. Nama bank di-join di dalam fungsi (klien admin di halaman publik tak diperlukan lagi). Token salah/belum dibagikan/dicabut = galat sama; hak eksekusi `anon, authenticated, service_role` diberikan ulang (PUBLIC dicabut).
- **Uji rollback:** anon menerima hanya kolom di atas (tanpa telepon, penghasilan, id agen, token); setelah dicabut dan untuk token acak ditolak; data uji nol.
- **Kode:** `getSharedDbr` memakai `bank_name` dari fungsi; jalur cadangan dan klien admin sudah dihapus (halaman publik tidak memakai service role lagi).

## `0162` — Pemimpin melihat listing organisasi + statistik organisasi tanpa listing pribadi (✅ DITERAPKAN 2026-09-26, diuji rollback sebelumnya)

- **Aturan akses baca:** `listings_select_published_or_owner_or_staff` dan policy select `listing_photos/videos/amenities` ditambah `OR (organization_id IS NOT NULL AND is_org_leader(organization_id))`. Pemimpin aktif organisasi bisa MELIHAT semua listing atas nama organisasinya (semua status); listing pribadi (organization_id NULL) tidak pernah terbuka; ubah/hapus/terbit/refresh tetap milik pembuat; `listing_leads` tetap hanya pemilik.
- **Perbaikan kebocoran statistik:** `agent_statistics_summary/daily` cakupan organisasi menghitung semua listing anggota termasuk yang pribadi (`top_listings` menampilkan judul listing pribadi ke pemimpin). Kini cakupan organisasi hanya menghitung listing berorganisasi itu (`listing_status`, `lead_pipeline`, `active/stale_listings`, `top_listings`, `members`, deret harian views/leads). Cakupan pribadi tidak berubah; `refresh` per anggota tetap agregat orang.
- **Uji rollback:** sebelum migration pemimpin melihat 0 listing organisasi; sesudah: hanya listing organisasi (bukan pribadi anggota, bukan organisasi lain) beserta fotonya; UPDATE oleh pemimpin 0 baris; anggota melihat listing miliknya; pemimpin organisasi lain hanya miliknya; pemimpin yang keluar 0. Statistik: fungsi lama menampilkan judul listing pribadi (aktif=2, views=3), fungsi baru hanya listing organisasi (aktif=1, views=1), cakupan pribadi tetap 2 dan 3. Data uji nol setelah rollback.
- **Kode:** `getMyListingsData` (pemimpin: semua listing organisasi + nama pembuat lewat `organization_roster`), `getMyListingDetail` (baca saja untuk pemimpin), `getAgentDashboard(userId, context)`. Tanpa 0162 kode tetap aman: pemimpin hanya melihat listing miliknya, detail listing anggota = tidak ditemukan.

## `0164` — Koreksi kepemilikan BYOK M13: role Agent ditambahkan (✅ DITERAPKAN 2026-09-26)

- **Masalah:** Gate `PRE-00-O_M13_PROVIDER_CATALOGUE_BYOK_AUTHORITY_GATE_FULL_v1.0.md` §23 (Actor/Role Reconciliation) eksplisit menetapkan Agent = "OWN — full own lifecycle" untuk `own_byok_connection.*`, Developer Partner = "No M13 ownership inferred". Migration `0009` (seed) + `0016` (RLS) mengimplementasikan kebalikannya — hanya Developer Partner yang dapat `m13.own_byok_connection.*`, Agent sama sekali tidak — mengikuti `STEP12-01_ROLE_PERMISSION_MASTER_MATRIX.csv` (baris frozen). Komentar tabel `agent_ai_connections` di `0016` bahkan menafsirkan ulang "agent" pada nama tabel sebagai istilah generik, bukan role platform Agent. Akibatnya Agent (persona utama app) tidak bisa memakai BYOK sama sekali, padahal `persona-nav.tsx` sudah menaruh menu "AI Assistant" di nav Agent (`/agent/ai`).
- **Keputusan pemilik produk (2026-09-26):** AI Assistant/BYOK dipakai dengan API key masing-masing; untuk MVP dibuka untuk **Agent dan Developer Partner saja** (bukan Manager/Instructor, supaya shell/nav area Admin dan Instructor tidak perlu diubah dulu).
- **Perbaikan:** `INSERT` 8 baris `role_permissions` (create/view/update/rotate/test/enable/disable/disconnect, scope `own`) untuk role `agent`. Developer Partner tidak dicabut (sudah ada dari 0009). Tidak ada perubahan skema/RLS/trigger — RLS `agent_ai_connections_*` (0016) sudah generik lewat `has_permission(code, user_id)`, otomatis mengikuti `role_permissions` manapun yang berlaku. `COMMENT ON TABLE agent_ai_connections` diperbarui supaya tidak lagi menyesatkan.
- **Diuji:** query `role_permissions` sebelum/sesudah — sebelum hanya `developer_partner`+`superadmin`, sesudah `agent`+`developer_partner` (scope `own`) dan `superadmin` (scope `all`) untuk kedelapan action. Advisor keamanan dicek setelah apply — tidak ada temuan baru dari migration ini.
- **Urutan deploy:** tidak ada kode yang bergantung pada migration ini (UI `/agent/ai` belum dibangun) — aman diterapkan kapan saja. Rollback: `DELETE FROM role_permissions WHERE role_id = (SELECT id FROM roles WHERE code = 'agent') AND permission_id IN (SELECT id FROM permissions WHERE action_code LIKE 'm13.own_byok_connection.%')`.
