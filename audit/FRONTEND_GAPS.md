...
1. **Alamat kembali setelah bayar: DISELESAIKAN pemilik 2026-09-26** lewat dashboard Midtrans (Finish Redirect URL = `https://staging.rumahagen.com/agent/komersial/pesanan`, tanpa perubahan kode). Pengaturan ini per lingkungan Midtrans: ULANGI di dashboard **Production** dengan alamat domain produksi saat go-live. Halaman Pesanan tetap tanpa polling; konfirmasi webhook bisa terlambat beberapa menit (ada catatan "muat ulang").
...
## 2026-09-26 — Fase 4b: Kalkulator DBR Agent (M07)

Tanpa migration dan tanpa perubahan API. Rute `/agent/dbr` (Kalkulator), `/agent/dbr/riwayat`, `/agent/dbr/riwayat/{id}`, dan halaman publik prospek `/dbr/shared/{token}`; kode: `lib/agent/dbr-{rules,types,data}.ts` (diuji), `components/agent/{DbrCalculator,DbrResultCard,DbrActions,DbrViews}.tsx`, `components/public/SharedDbrView.tsx`. Pintasan "Kalkulator DBR" di Dashboard kini aktif. Contoh: `/komponen/agent?layar=dbr|dbr-riwayat|dbr-detail|dbr-publik&keadaan=...`. Alur hitung/bagikan BELUM diuji dengan login (uji manual pemilik); alur token publik + cabut diuji rollback SQL.
0. **2 bank contoh diisi 2026-09-26 untuk uji coba** ("Bank Contoh A (Uji Coba)" ambang 35% bunga 8,5%; "Bank Contoh B (Uji Coba)" ambang 40% bunga 7,9%; nama fiktif dan berlabel, BUKAN ambang bank sungguhan): hapus/nonaktifkan dan ganti dengan bank sungguhan lewat Admin (Fase 5) atau `PUT /banks/{id}` sebelum dipakai agen sungguhan.
1. **(Riwayat) Bank Master kosong (0 bank) dan belum ada layar Admin:** kalkulator menampilkan "Belum ada bank yang tersedia" sampai staf mengisi bank (nama, ambang DBR, bunga). API `POST /banks` ada (Admin/Superadmin); layar Admin Bank Master = Fase 5. Angka ambang/bunga adalah data bisnis, tidak saya isi.
2. **Kebocoran di API publik: DITUTUP migration 0163 (diterapkan 2026-09-26).** Semula: `GET /calculator/dbr/shared/{token}` mengembalikan SELURUH baris simulasi kepada siapa pun yang memegang token (agent_id, listing_id, bank_id, share_token, nomor telepon prospek, penghasilan, cicilan berjalan). Halaman publik saya memilih bidang sendiri di server dan tidak memakai rute itu. Usul: batasi keluaran RPC `get_shared_dbr_simulation` (migration) dan rute; tunggu persetujuan.
3. **Isi halaman berbagi (keputusan default, bisa diubah):** hanya hasil DBR, status, ambang, bank, plafon, cicilan, tenor, bunga, harga, DP, dan nama prospek (bila ada). Penghasilan, cicilan berjalan, nomor telepon, dan identitas agen tidak tampil. Nama bank dibaca dengan klien admin server-only karena `banks` tertutup untuk anon (diuji).
4. **Membuat link baru mengganti link lama** (token baru; RPC `share_dbr_simulation`), jadi dialog memperingatkan dan memakai label "Buat Link Baru" saat link aktif. Mencabut = seketika (diuji: setelah cabut anon ditolak).
5. **Tenor hanya 10/15/20 tahun** (wireframe; API menerima angka bulan apa saja). Kaitan ke listing (`listing_id`) dan `pdf_export_url` tidak dipakai. Ekspor PDF dibuat on-the-fly oleh API (memuat penghasilan dan cicilan berjalan untuk agen).
...
## 2026-09-26 — Fase 4f: Koneksi AI Saya + AI Assistant (M13 BYOK)

Tanpa migration baru untuk UI ini (migration 0164 sebelumnya sudah menambahkan permission `m13.own_byok_connection.*` untuk role Agent). Rute `/agent/ai` (daftar koneksi + Tambah Koneksi) dan `/agent/ai/asisten` (chat); kode: `lib/agent/ai-{rules,data}.ts` (aturan diuji), `components/agent/{AiConnectionsView,AddAiConnectionDialog,AiConnectionRowActions,AiAssistantView}.tsx`. Data koneksi/katalog dibaca langsung dari Supabase (RLS pemanggil) di server, mutasi (Tambah/Test/Putuskan/chat) lewat `/api/ai-connections*` dan `/api/ai-assistant/chat`. Riwayat chat hanya di memori (tidak ada tabel penyimpanan pesan). **Belum diuji dengan login** — sandbox pengembangan ini tidak punya kredensial Supabase (`NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY`) untuk menjalankan `next dev`, jadi hanya diverifikasi lewat `npm run build` (semua rute berhasil, termasuk dua rute baru), `npm run typecheck`, dan `npm test` (254 lulus, termasuk 4 baru) — bukan uji visual di browser.
1. **Katalog `ai_providers` kosong di DB live** (dicek langsung, 0 baris) — sesuai catatan migration 0015 yang sengaja tidak menyeed provider nyata. Layar Koneksi AI Saya sudah menangani ini (pesan "Belum ada provider AI di katalog..."), tapi secara praktis Tambah Koneksi tidak bisa dipakai sampai Superadmin mengisi lewat `POST /ai-providers` — **belum ada UI Admin untuk ini** (`/admin/provider-ai` ada di menu tapi halamannya belum dibangun, wireframe: `02-Admin/M13-Provider-Catalogue.dc.html`).
2. **`ai_providers` tidak punya kolom penanda "butuh kredensial kedua"** (identifier publik + secondary key, migration 0080 — kolom itu ada di `agent_ai_connections`, bukan di katalog). Wireframe menampilkan field itu kondisional per provider (mis. hanya Cloudinary); karena tidak ada data untuk menentukan provider mana yang butuh, dialog Tambah Koneksi menampilkan kedua field itu sebagai opsional untuk SEMUA provider (bukan ditebak dari nama/kode provider).
3. **"Test Koneksi" hanya ditampilkan untuk status `unverified`** (uji ulang koneksi `active` tidak diekspos di UI ini) meski endpoint `/ai-connections/{id}/test` menerimanya — mengikuti wireframe apa adanya.
4. Wireframe menaruh AI Assistant sebagai satu-satunya rute nav Agent (`/agent/ai`, gabungan Koneksi + Chat); di sini dipecah jadi dua rute (`/agent/ai` untuk kelola koneksi, `/agent/ai/asisten` untuk chat) mengikuti pola sub-rute modul lain (mis. `/agent/dbr/riwayat`) — `persona-nav.tsx` tetap menunjuk ke `/agent/ai` (Koneksi AI sebagai halaman utama, tombol "Buka AI Assistant" menuju sub-rute).

## 2026-09-26 — Fase 4 (terakhir): Kualifikasi — Bukti/Evaluasi/Title (M15)

Tanpa migration dan tanpa perubahan API. Rute `/agent/kualifikasi` (Bukti + Ajukan Bukti Baru), `/agent/kualifikasi/evaluasi` (Status Evaluasi & Penghargaan + Ajukan Banding), `/agent/kualifikasi/presentasi` (Presentasi Title di profil publik); kode: `lib/agent/qualification-{rules,data}.ts` (aturan diuji), `components/agent/{QualificationEvidenceView,SubmitEvidenceDialog,QualificationEvaluationView,AppealDialog,TitlePresentationView}.tsx`. Pintasan "Kualifikasi & Penghargaan" di Dashboard kini aktif (sebelumnya nonaktif menunggu layar ini). Data dibaca langsung dari Supabase (RLS pemanggil, scope OWN); mutasi lewat `POST /qualification-evidence`, `POST /awards/{id}/appeals`, `PUT /agents/me/awards/presentation` (RPC `set_my_public_titles`, 0148). **Fase 4 (Agent lanjutan) selesai** — sisa Fase 5 (Admin).
1. **Kolom "Title/Jalur Dituju" di tabel evaluasi bersifat turunan, bukan kolom asli.** `qualification_evaluations` tidak punya referensi title langsung (hanya `awarding_path_version_id`/`awarding_rule_version_id`, keduanya NULLABLE FK ditunda). Untuk evaluasi yang sudah menghasilkan award, label diambil dari `award_instances.title_definitions.name` (otoritatif). Untuk evaluasi yang BELUM menghasilkan award, label diambil best-effort dari `qualification_evidence.evidence_payload.target_title` milik evidence yang mengarah ke evaluasi itu (field bebas yang diisi Agent saat mengajukan bukti, bukan tervalidasi) — bila tidak ada, tampil "—". Evaluasi yang dibuat staf tanpa evidence yang mengarah ke sana akan selalu tampil "—".
2. **"Test Koneksi"-nya M15 (banding admin decide) belum ada UI-nya** — `PATCH /awards/{id}/appeals/{appealId}/decide` (approve/reject) murni tugas staf, disengaja ditunda ke Fase 5 Admin bersama layar Jalur Penghargaan/Banding Penghargaan yang sudah ada di menu Admin.
3. Katalog `title_definitions` sudah ada 1 baris nyata ("Agen Tersertifikasi Etika & Komunikasi") — tidak seperti `ai_providers` M13, tidak perlu diseed manual untuk mulai uji coba.
4. **Belum diuji dengan login Agent** — sandbox pengembangan ini tidak punya kredensial Supabase untuk `next dev`; hanya diverifikasi lewat `npm run build` (semua rute baru lulus), `npm run typecheck`, dan `npm test` (262 lulus, 12 baru).
